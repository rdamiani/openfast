...
!=======================================================================
SUBROUTINE UserYawCont ( YawPos, YawRate, WindDir, YawError, NumBl, ZTime, DT, DirRoot, YawNeut, YawPosCom, YawRateCom )
...
USE                             Precision
USE                             NWTC_Library
USE   ElastoDyn_Parameters, only: DOF_yaw               !RRD added
...
REAL(ReKi), INTENT(INOUT)    :: YawNeut                           ! This is from SrvD; We set it to the current YAW angle to simulate a stop by enbaling a large stiffness and damping #RRD added 

CHARACTER(1024), INTENT(IN ) :: DirRoot                                         ! The name of the root file including the full path to the current working directory.  This may be useful if you want this routine to write a permanent record of what it does to be stored with the simulation results: the results should be stored in a file whose name (including path) is generated by appending any suitable extension to DirRoot.

LOGICAL,    SAVE            :: SFLAG  = .TRUE.                 !RRD: this flag will keep track of first call to this routine, so that we can set the YawNeut to the current YawPos only once, at the first call. This is needed to simulate a stop by enbaling a large stiffness and damping #RRD added
...
!Note that IF  ( ZTime >= p%TYCOn ): !This check is done by servodyn, no need to do it here
     YawRateCom =  0.0
     if (SFLAG) then
         SFLAG = .FALSE. !RRD: set the flag to false so that we do not set the YawNeut again
         YawNeut    = YawPos
     endif   
     YawPosCom  = YawNeut
     !write(*,*),'YawPos, YawNeut [deg] YawposCom  =',YawPos*R2D, YawNeut*R2D, YawposCom !debug
!END IF
